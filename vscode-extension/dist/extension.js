...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("codeyz.chatView", provider));
    context.subscriptions.push(vscode.commands.registerCommand("codeyz.openChat", async () => {
        await vscode.commands.executeCommand("workbench.view.extension.codeyz");
    }));
}
function deactivate() { }
class CodeyzViewProvider {
    ...
    getConfig() {
        ...
        const serverUrl = (cfg.get("serverUrl") || "http://127.0.0.1:8765").replace(/\/$/, "");
        const token = cfg.get("localToken") || "";
